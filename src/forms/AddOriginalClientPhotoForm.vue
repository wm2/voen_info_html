  ...
  <v-dialog v-model="dialog" persistent max-width="600px">
    <template v-slot:activator="{ on, attrs }">
      <v-list-item-content
          ...
          v-bind="attrs"
          v-on="on">
        <v-btn color="success"  >+ Оригиналы</v-btn>
      </v-list-item-content>
    </template>
    <v-card>
      <v-card-title>
        <span class="headline">+ Оригиналы</span>
      </v-card-title>
      <v-card-text>
        <v-container>
...
export default {
  ...
  methods: {
    ...
    upload() {
      ...
      this.axios.post(process.env.VUE_APP_BACKENDURL + '/api/v1/report/' + this.report_id + '/original/add',
          formData,
          {
            headers: {
